...
class TC1TCPScan(TestCase):
    ...
    def run(self, context):
        ...
        # 3. Run nmap TCP SYN scan: SINGLE PROBE PER PORT (no retries)
        nmap_cmd = (
            f"sudo nmap -sS -p- -Pn -n -T5 --max-retries=0 "
        ...
        if open_ports:
            print(f"[+] Total: {len(open_ports)} open TCP ports. Classifying & capturing evidence...")
        else:
